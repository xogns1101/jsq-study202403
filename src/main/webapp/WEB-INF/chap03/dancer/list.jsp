...
<%--
    //  디스패처에 의해 request 객체가 이쪽으로 전달 됬을 것입니다.
    // request 에 담겨있는 댄서 리스트를 꺼내는 메서드.
    List<Dancer> dancerList = (List<Dancer>) request.getAttribute("dl");

    // MVC 구조에서는 View 역할을 하는 jsp 쪽에서 자바코드를 최소화 하기로 약속함.
    // el을 통해서 request 에 있는 dl 이라는 이름의 데이터를 꺼내 쓰도록 하자.
--%>
...
<body>
    ...
    <ul id="dancer-list">
        ...
        <%--
            c:forEach 태그를 이용하여 향상 for 문을 구현하는 법.
            제어변수를 선언하시고, items 속성에 반복문을 돌리고자 하는 컬렉션 자료형을 세팅.
            request 객체 내에 있는 dl 이라는 이름의 데이터를 el 을 사용하여 꺼낼 수 있다.
            ${requestScope.dl} -> 요게 원형인데, Scope 를 언급하지 않으면 dl 이라는 이름의 데이터를
            전달된 모든 객체에서 뒤져서 꺼내 옵니다.
        --%>
        <c:forEach var="d" items="${dl}">
            <li>
            <%-- el을 사용하여 객체를 지목한 후 메서드를 호출할 때는 get 을 제외한 나머지 이름을 애기하면 됩니다. --%>
                알아서 getter 를 호출해 줍니다. (필드명) -- %>
    ...
    <script>
        ...
        $dancerList.onclick = e => {
            ...
            // 링크로 삭제 요청하면 get 요청이 들어감 -> 삭제는 post 로 전달해야 합니다.
            // 자바스크립트로 form 없이 post 요청 보내는 방법은 추후에 알려드리겠습니다.
